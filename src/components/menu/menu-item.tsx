  ...
  return (
    <article className="flex items-center gap-4 p-4 rounded-lg bg-card hover:bg-muted/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-manrope font-bold text-lg text-foreground mb-1">{t(item.nameKey)}</h3>
        <p className="text-sm text-muted-foreground leading-relaxed">{t(item.descriptionKey)}</p>
      </div>
      <span className="font-bold text-lg text-foreground flex-shrink-0">{item.price}</span>
    </article>
  )
}
